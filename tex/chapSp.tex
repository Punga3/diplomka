...
\begin{defi}
Let $m\leq n$, $v,u,w\in\set{m}$ such that $v\neq u$, $v\neq w$ and let $T\in\T_{nbtree}$ with root labeled by $v$. By $T_{v?=\{u,w\}}$ we denote the subtree of $T$ whose root is the node connected to $v$ by the edge $\{u,w\}$.
\end{defi}
...
\begin{align}
\Pr_{\G\in\G_m}[T_{0?=\{u\}}\text{ fails}|v?=\{u\}]=\Pr_{\G_{m-1}}[\tilde T_{0?=\{u\}}].
\end{align}
\end{lemm}
...
\begin{align}
\Pr_{G\in\G_m}[\text{$T$ fails}]&=\sum_{u\in V\setminus\{0\}}\Pr_{G\in\G_{m}}[E(0,u)]\Pr_{G\in\G_{m}}[T_{0?=\{u\}}\text{ fails}|E(0,u)]\\
&\geq \Pr_{G\in\G_{m}}[T_{0?=\{u\}}\text{ fails}|E(0,u)]\\
