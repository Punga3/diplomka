...
\end{thrm}
\begin{proof}
If any two trees $T_\alpha$ and $T_\beta$ computed the witnesses $\alpha,\beta$ of the formula $\Gamma(x,y)$ on some subset of $\Omega$ we can combine then into one tree that outputs an edge one the same subsets $\Omega$, so we can just analyze the case where the witnesses are computed by the same tree. Let $T\in F_{rud}$ be a tree of depth $n^{1/t}$, for some $t>\mathbb{N}$, that outputs a pair vertices.
...
\begin{exam}
Let $\mathcal{G}_k$ consist of those graphs on the vertex-set $k$ which contain the edge $E(0,1)$ and then has exactly one other edge. As $k$ increases, the fraction of edges gets smaller than any standard positive fraction. But
...
\begin{lemm}[The hereditary property of 0-1 sentences]\label{lemmhere}
