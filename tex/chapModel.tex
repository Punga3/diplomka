...
(TODO: 0-1 laws for subsequences of $\ALL_k$, Portions, Large portions)

The wide sequence $\ALL_k$ is maximal in the sense that for every $\G_k$ we have that $\G_{k}\subseteq \ALL_{g_k}$. Since we proved 0-1 law for $\ALL_k$ we can ask whether this tells us anything about the subobjects of $\ALL_k$ since they consist in some some of all other wide sequences.

\begin{defi}
Let $\G_k$ be a wide sequence, we say that $\G_k'$ is a \textbf{portion} of $\G_k$ if we have $\G_k'\subseteq \G_k$ for all $k$ big enough, we write $\G_k'\leq G_k$. We say it is a \textbf{large portion} if we have
\[\lim_{k\to\infty}\frac{\abs{\G_k'}}{\abs{\G_k}}>0,\]
...
However each $\EDGE_k$ and $\nEDGE_k$ have complete theories. So natural follow up to this question whether 0-1 law holds for all isomorphism closed categorical wide sequences. We present a partial result.

(TODO: Requirements for a counterexample - non-sparse, non-dense, automorphisms)

\begin{thrm}
Most isocats are 0-1.
\end{thrm}
\begin{proof}
Partition $\ALL_k$.
\end{proof}
