...
\begin{lemm}[The hereditary property of open sentences]\label{lemmhere}
Let $\G_k'\leqp \G_k$ be wide sequences, let $\varphi(\overline x)$ be an open $\{E\}$-sentence and $\alpha\in F$ such that
\[\lim_{F}\G_n(\overline \alpha)\bbl\varphi^\Gamma\bbr\in\{\0,\1\}\]
then
\[\lim_{F}\G_n'\bbl\varphi(\overline \alpha)\bbr = \lim_{F}\G_n\bbl\varphi(\overline \alpha)\bbr.\]
\end{lemm}
...
(TODO: Example?, Major portion, decomposition)

We will use this lemma in Chapter \ref{chapsparse} that.
