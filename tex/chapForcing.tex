...
\begin{defi}
Let $L\subseteq L_{all}$ and let $F$ be a non-empty set of $\M$-finite functions which are elements in $\M$. We call it an \textbf{$L$-closed family} if it satisfies the following:
\begin{itemize}
\item The domain of any function $\alpha\in F$ is $\Omega$ and the range is $\M$.
